 ...
 *
 * Usage:
 *   - For local MongoDB: npx tsx src/scripts/migrations/rename-board-type-to-visibility.ts
 ...
 *   - .env.{NODE_ENV} file exists with DB_URI, DB_NAME, and DB_BOARDS_COLLECTION configured
...
const DB_BOARDS_COLLECTION = envConfig.dbBoardsCollection
...
async function migrateBoardTypeToVisibility(): Promise<void> {
  if (!DB_URI || !DB_NAME || !DB_BOARDS_COLLECTION) {
    throw new Error(
      'Required environment variables are not set. Please check DB_URI, DB_NAME, and DB_BOARDS_COLLECTION.'
    )
  }
