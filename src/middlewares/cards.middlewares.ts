  ...
  custom: {
    options: (value, { req }) => {
      ...
      if (attachmentType === AttachmentType.File) {
        ...
        if (!hasAllFields) {
          throw new Error(`${CARDS_MESSAGES.ATTACHMENT_FILE_MISSING_REQUIRED_FIELDS}: ${requiredFields.join(', ')}`)
        }
      }
  ...
  custom: {
    options: (value, { req }) => {
      ...
      if (attachmentType === AttachmentType.Link) {
        ...
        if (!hasAllFields) {
          throw new Error(`${CARDS_MESSAGES.ATTACHMENT_LINK_MISSING_REQUIRED_FIELDS}: ${requiredFields.join(', ')}`)
        }
      }
  ...
  checkSchema(
    {
      user_id: {
        ...
        custom: {
          options: async (value, { req }) => {
            if (!ObjectId.isValid(value)) {
              throw new Error(CARDS_MESSAGES.INVALID_USER_ID)
            }
            ...
            if (!user) {
              throw new Error(CARDS_MESSAGES.USER_NOT_FOUND)
            }
...
export const memberIdValidator = validate(
  checkSchema(
    {
      user_id: {
        custom: {
